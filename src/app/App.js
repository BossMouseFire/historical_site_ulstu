    ...
    return (
      <div>
        <nav className="navbar">
          ...
          <button onClick={() => this.changeState('attractions')}>ДОСТОПРИМЕЧАТЕЛЬНОСТИ</button>
          <button onClick={() => this.changeState('map')}>ХРОНОКАРТА</button>
          <button onClick={() => this.changeState('greatPeople')}>ВЫДАЮЩИЕСЯ ЗЕМЛЯКИ</button>
        </nav>
        {this.state.map ? <Quiz /> : <GreatPeople/>}
